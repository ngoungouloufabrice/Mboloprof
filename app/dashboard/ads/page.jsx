...
export default function MyAdsPage() {
    ...
    const fetchMyAds = async () => {
        if (!user?.id) return;
        try {
            const data = await adService.getByTeacher(user.id);
            setAds(Array.isArray(data) ? data : []);
        } catch (err) {
            console.error("Erreur fetch ads:", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Box>
            ...
            <Box className="mb-10 flex items-center justify-between">
                <Box>
                    ...
                    <Typography className="text-gray-500 font-medium">
                        Gérez vos cours publiés et suivez leurs performances.
                    </Typography>
                </Box>
                <Link href="/publish" className="no-underline">
                    <Button
                        variant="contained"
                        startIcon={<AddIcon />}
                        className="bg-[#2463eb] hover:bg-blue-700 px-6 py-3 rounded-xl text-[14px] font-black normal-case shadow-lg shadow-blue-200"
                    >
                        Nouvelle Annonce
                    </Button>
                </Link>
            ...
            {loading ? (
                ...
                <Paper elevation={0} className="p-16 rounded-[32px] border border-gray-100 shadow-sm text-center">
                    <Box className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6">
                        <AdsIcon className="text-gray-300 text-4xl" />
                    </Box>
                    <Typography variant="h5" className="font-black text-[#111827] mb-2">
                        Aucune annonce publiée
                    </Typography>
                    <Typography className="text-gray-400 font-medium mb-8">
                        Commencez par créer votre premier cours pour attirer des étudiants.
                    </Typography>
                    <Link href="/publish" className="no-underline">
                        <Button
                            variant="outlined"
                            className="border-[#2463eb] text-[#2463eb] font-black px-8 py-3 rounded-xl hover:bg-blue-50"
                        >
                ...
                <Box className="space-y-4">
                    ...
                    <Box className="hidden md:flex px-8 py-4 bg-gray-50 rounded-2xl mb-4">
                        <Typography className="flex-[2] text-[12px] font-black uppercase tracking-widest text-gray-400">Cours / Titre</Typography>
                        <Typography className="flex-1 text-[12px] font-black uppercase tracking-widest text-gray-400">Matière</Typography>
                        <Typography className="flex-1 text-[12px] font-black uppercase tracking-widest text-gray-400 text-center">Prix (mois)</Typography>
                        <Typography className="flex-1 text-[12px] font-black uppercase tracking-widest text-gray-400 text-center">Actions</Typography>
                    </Box>

                    {ads.map((ad) => (
                        <Paper key={ad._id} elevation={0} className="p-6 md:p-8 rounded-[24px] border border-gray-100 shadow-sm hover:shadow-md transition-all">
                            <Box className="flex flex-col md:flex-row items-center gap-6">
                                <Box className="flex-[2] flex gap-4 items-center">
                                    ...
                                    <Box>
                                        ...
                                        <Typography className="text-[12px] text-gray-400 font-bold mt-1">
                                            {ad.level} • {ad.city}
                                        </Typography>
                                    </Box>
                                </Box>
                                ...
                                <Box className="flex-1 flex justify-center gap-2">
                                    <Link href={`/dashboard/ads/edit/${ad._id}`}>
                                        <IconButton className="bg-blue-50 text-blue-600 hover:bg-blue-100 p-2.5 rounded-xl">
            ...
            >
                <DialogTitle className="font-black text-xl text-center">Supprimer l'annonce ?</DialogTitle>
                <DialogContent>
